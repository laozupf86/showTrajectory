...
public class TestForBasic {
	...
	public static void main(String[] args) {
		...
		basicTable.BasicTable bt;
		deltaBasicTable.BasicTable dbt;
	    ...
	    while(true){
	    	...
	    	try {
				...
				if (n.equals("1")){
					bt = new basicTable.BasicTable();
					
					for (String s : commonMethods.LoadFromFile.getIDs(path, "")){
						bt.InsertSingleRecord(i, commonMethods.LoadFromFile.getPoints(path, s, ""));
					...
					dbt = new deltaBasicTable.BasicTable();
				/*	for (String s : staticFunction.LoadFromFile.getIDs(path, "")){
						dbt.InsertSingleRecord(i, staticFunction.LoadFromFile.getPoints(path, s, ""));
		...
		//ArrayList<ArrayList<SamplePoint>> sp = new ArrayList<ArrayList<SamplePoint>>();
		//sp = staticFunction.LoadFromFile.getArrayPoints(path, "");
		int i = 0;
		for (int j = 1;  j < 2; j++){
			path = "E:\\uqhwan16\\data\\cleaned\\" + j + "\\";
